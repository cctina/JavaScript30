...
(function () {
	...
	const control = {
		init: function () {
			view.init();
			this.tick();
			view.clockElements.forEach(function (item) {
		...
		tick: function () {
			...
			console.log(`${this.hour}:${this.min}:${this.sec}, hour deg = ${model.degree.hour-90}`);
	...
	const view = {
		init: function () {			
			...
			this.clockElements = [];
			this.clockElements.push(document.querySelector('.clock'));
			this.clockElements.push(document.querySelector('.clock-face'));
			this.clockElements.push.apply(this.clockElements, document.querySelectorAll('.hand'));
			this.render();
		},
